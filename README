...
* `#store_external_table` - mimics a call to the *Semantic MediaWiki* extension's
  `#subobject` parser function onto each row retrieved by any of the `#get_*_data`
  functions, in order to store the entire array of data semantically
...
This version of the External Data extension requires MediaWiki 1.33 or
higher.
